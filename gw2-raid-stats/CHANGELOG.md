...
- **Tier-2 boon tracking** (Might, Fury, Regeneration, Protection, Swiftness) — alongside Quickness and Alacrity:
  ...
  - The session **"Session Stats" tab is renamed "Boon Uptime"** and rebuilt as per-encounter **heatmap cards**: each encounter card has its own column labels (no sticky-header needed), with color-tinted cells for all 7 boons per sub. A panel-level **Sub averages / All players** toggle switches every card between sub-average rows and full per-player rows (grouped under their sub). A red-outlined cell marks the "one bad apple" case — sub average is fine but exactly one member is in the red. Each sub's booner line names the Quickness and Alacrity providers and tags which one was the healer.
