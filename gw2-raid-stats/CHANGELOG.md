...
### Fixed
- **HTCM prog session dates no longer show a day late.** `encounter_time` is stored as a pure UTC instant and the containers run in UTC, so an evening Central-time raid (8–11pm ≈ 01:00–04:00 UTC) was grouped and labelled under the *next* day's date. All HTCM session dating — the prog page's session list/selector, session detail windows, phase-DPS and mechanic trends, and the Discord HTCM summary's session keys — now goes through a raid-night timezone (`HtcmProgService.SessionTimeZone`, America/Chicago), so a session is a raid night on the guild's calendar. A raid that runs past local midnight also stays one session instead of splitting in two.
...
### Changed
- **HTCM prog page reorganised into tabs.** The single ever-growing page is now **Session / Trends / Attendance**: the session view keeps the selector, KPI row, pulls table and insight panels with the Phase DPS comparison and session list alongside; the Phase DPS Trends and Mechanic Trends tables move to the Trends tab.
...
### Added
- **Squad Builder: named non-guildies.** Guests can now be added by name (not just as anonymous "Pug DPS") — type a name in the new **Non-guildie name** field and they take a DPS slot like a pug, but keep their name in the squad grid and the Discord post. Guests can also be assigned to any slot manually via the slot picker; removing a guest chip clears them from the grid.
- **Squad Builder: strikes.** All 14 strike missions (Icebrood Saga through Janthir Wilds) are now selectable bosses, with correct Elite Insights trigger IDs, and appear in the Mechanic Role Catalog so mechanic roles can be defined for them like raid bosses. The **Random Bosses** roll gains a **Strikes** entry in the wing filter (an empty filter still means raid wings only). The boss picker is regrouped into per-wing / per-expansion rows with compact chips so the larger roster stays scannable.

- **HTCM attendance.** New **Attendance** tab on the HTCM prog page: per guild member, the number of prog *nights* attended (with % of all nights — nights, not log counts), total pulls, how many of their pulls reached at least **Giants / Saltspray / Soo-Won** (by the pull's furthest phase on the canonical ordering), and their last night. Backed by `GET api/htcmprog/attendance`.
- **Burst target tiles on the HTCM prog page.** The session view now opens with three at-a-glance tiles — Timecaster / Giants / Saltspray — showing tonight's session-average squad burst DPS coloured against the guild targets (175k / 310k / 170k, the same targets the Discord summary judges), plus pulls-reached and average phase duration.
...
- **Manual cut line in the Squad Builder.** Each boss after the first now has a **Cut roles here** button in the Mechanics list. Cutting re-randomizes the squad from that boss onward with no locks, so base roles can fully reshuffle mid-session (useful when a strike or later boss wants a different composition than the rest of the run). Reuses the existing reset-segment banners and the Discord "Mid-set Swaps" output. This generalizes the previous reset, which only appeared on a mechanic-coverage conflict.
- **HTCM prog page — per-player phase insights and burst comparison.** A new `Phase Insights` panel under each session breaks each guildie's deaths, dead-at-phase-start count, and average Debilitated uptime % across the three burst-comparison phase groups: **Void Time Caster**, **Giants** (Void Giant 1 + Void Giant 2, breakbar sub-phases excluded so concurrent damage isn't double-counted), and **Void Saltspray Dragon**. Each expanded pull also gets a **Burst Comparison** table — duration, squad DPS, and per-player DPS across the same three groups so it's obvious when the squad bursts unevenly. Plus a **Session Burst Averages** panel that aggregates the same metrics across every pull in the session (per-player DPS uses each player's actual time in the phase, so missed pulls don't dilute their average). Migration 032 adds the underlying `player_encounter_phase_stats` table (HTCM-only, ~120 rows per pull). EI's per-phase Defenses + BuffUptimesActive + deadCombatTimes (newly parsed) drive the metrics; Debilitated buff ID 67972. Backfill historical HTCM pulls via **Admin → Manage Logs → Rescan**.
...
- **Event signups auto-close at start time.** Once an event's scheduled time passes, the bot transitions its status from `Scheduled` to `Closed`, re-renders the Discord embed with all buttons greyed and a 🔒 CLOSED — title prefix, and rejects further button clicks with "signups are closed — the session has started". The attendance roster is preserved as a frozen record. Closing is driven by the existing `EventReminderProcessor` background poller (60s tick, so close lag ≤ 60s past start time). Cancelled events still take precedence over Closed if the admin cancels first.
...
### Fixed
- **Discord squad post — Mechanics and Mid-set Swaps no longer truncated.** Both sections were stuffed into a single embed field, which Discord caps at 1024 characters — long squads lost the tail of the boss list (e.g. "Twin Largos • Tan..." with Dhuum missing). Now each boss gets its own full-width mechanics field, prefixed with its order number in the squad. Bosses with no mechanic assignments are kept in the list with a `(no mechanic assignments)` note so the numbering stays consecutive and the commander can spot what still needs assigning. Each reset segment also gets its own swaps field.
