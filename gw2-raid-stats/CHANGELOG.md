...
- **Tier-2 boon tracking** (Might, Fury, Regeneration, Protection, Swiftness) — alongside Quickness and Alacrity:
  ...
  - The session **"Session Stats" tab is renamed "Boon Uptime"** and rebuilt as per-encounter **heatmap cards**: each encounter card has its own column labels (no sticky-header needed), with color-tinted cells for all 7 boons per sub. Click a sub to expand its per-player rows inline. A red-outlined cell marks the "one bad apple" case — sub average is fine but exactly one member is in the red.
