...
- **Boon Coverage report**:
  - **Home → Previous Session** now has two tabs: **Logs** (existing encounter table) and **Session Stats** (new). Session Stats shows per-encounter Quickness and Alacrity uptime broken out by sub-group, plus the booners who provided each boon with their generation %. Cells are colored green ≥ 95%, yellow ≥ 80%, red < 80%. A ⚠ flag next to a booner means their generation was < 60% — they were probably the bottleneck for that fight.
  - **Player Profile** gets a new **Boon Coverage** card that splits each player's history two ways: **Generation** (the sub's avg uptime on encounters where they were tagged as the booner — "how well did your booning hold up?") and **Self** (their own uptime on encounters where someone else booned — "did you stay in range?"). Includes per-boss and per-profession × role breakdowns so positioning issues on specific classes/specs surface immediately.
